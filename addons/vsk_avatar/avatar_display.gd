...
func _setup_avatar_eyes(_p_avatar_node: Node3D, p_skeleton: Skeleton3D) -> void:
	...
	if eye_spatial:
		...
		eye_global_transform = (node_util_const.get_relative_global_transform(
			avatar_node, eye_spatial
		))
	else:
...
func _setup_avatar_mouth(p_avatar_node: Node, p_skeleton: Skeleton3D) -> void:
	...
	# Mouth
	var mouth_spatial: Node3D = p_avatar_node.get_node_or_null(
		p_avatar_node.mouth_transform_node_path
	)
...
func _setup_hand_poses(p_avatar_node: Node, p_skeleton: Skeleton3D) -> void:
	if p_skeleton:
		...
		animation_player.add_animation_library(
			&"", load("res://addons/vsk_avatar/hand_pose_library.tres")
		)
