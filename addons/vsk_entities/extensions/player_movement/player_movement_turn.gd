...
func execute(p_movement_controller: Node, p_delta: float) -> bool:
	...
	get_xr_origin(p_movement_controller).transform = (
		get_xr_origin(p_movement_controller).transform
		* camera_position_transform
		* Transform3D(Basis().rotated(Vector3.UP, rot_offset), Vector3())
		* camera_position_transform.inverse()
	)
