...
static func set_relative_global_transform(
	p_root: Node3D, p_node3d: Node3D, p_gt: Transform3D
) -> void:
	if p_node3d.get_parent() == p_root:
		p_node3d.set_transform(p_gt)
	else:
		p_node3d.set_transform(
			get_relative_global_transform(p_root, p_node3d.get_parent()).affine_inverse() * p_gt
		)
