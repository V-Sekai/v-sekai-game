@tool
class_name SarXRComponentCameraMeshPlane
extends MeshInstance3D

# This probably doesn't work correctly for XR cameras, but keeping it here
# for aspirational purposes.

static func _xform_plane(p_transform: Transform3D, p_plane: Plane) -> Plane:
...
static func _get_endpoints_for_camera(p_camera: Camera3D) -> PackedVector2Array:
	var planes: Array = p_camera.get_frustum()

	var camera_gt_inv: Transform3D = p_camera.global_transform.inverse()

	var near_plane: Plane = _xform_plane(camera_gt_inv, planes[0])
	var far_plane: Plane = _xform_plane(camera_gt_inv, planes[1])
	# The is a placeholder for the plane index 2.
	var top_plane: Plane = _xform_plane(camera_gt_inv, planes[3])
	var right_plane: Plane = _xform_plane(camera_gt_inv, planes[4])
...
func _update_plane(p_lerp: float) -> void:
	var camera: Camera3D = get_parent()
	if camera:
		var sizes = _get_endpoints_for_camera(camera)
		if sizes.size() > 0:
