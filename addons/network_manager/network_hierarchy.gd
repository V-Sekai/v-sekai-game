...
static func write_entity_parent_id(p_writer: Object, p_entity_hierarchy_node: Node) -> Object:
	if p_entity_hierarchy_node and p_entity_hierarchy_node.get_entity_parent():
		p_writer = encode_parent_id(
			p_writer,
			p_entity_hierarchy_node.get_entity_parent().network_identity_node.network_instance_id
		)
	else:
...
static func write_entity_attachment_id(p_writer: Object, p_entity_hierarchy_node: Node) -> Object:
	if p_entity_hierarchy_node:
		p_writer = encode_attachment_id(
			p_writer, p_entity_hierarchy_node.cached_entity_attachment_id
		)
	else:
	...
	if sync_parent:
		...
		if sync_attachment:
			if entity_node.hierarchy_component_node.get_entity_parent():
				p_writer = write_entity_attachment_id(
					p_writer, entity_node.hierarchy_component_node
				)
	return p_writer
	...
	if entity_node:
		...
		if parent_id != network_entity_manager_const.NULL_NETWORK_INSTANCE_ID:
			if NetworkManager.network_entity_manager.network_instance_ids.has(parent_id):
				var network_identity: Node = (
					NetworkManager
					. network_entity_manager
					. get_network_identity_for_instance_id(parent_id)
				)
				if network_identity:
