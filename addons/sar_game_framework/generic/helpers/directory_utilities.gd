@tool
class_name SarDirectoryUtilities
...
## Returns are array containing the names of all the files directory of
## at the specific path.
## p_path is the path to the directory to search in.
...
static func delete_dir_and_contents(
	p_directory: DirAccess, current_dir_path: String, p_delete_root: bool
) -> int:
	if p_directory.list_dir_begin() != OK:
		push_error("Failed to list directory.")
		return FAILED
