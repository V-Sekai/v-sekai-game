...
var signal_table: Array = [
	...
	{
		"singleton": "NetworkManager",
		"signal": "entity_network_id_registered",
		"method": "_network_id_registered_added"
	},
	{
		"singleton": "NetworkManager",
...
func _network_id_unregistered_added(p_entity_id: int) -> void:
	if network_manager.is_server():
		...
		if network_entity_ids_pending_spawn.has(p_entity_id):
			network_entity_ids_pending_spawn.remove_at(
				network_entity_ids_pending_spawn.find(p_entity_id)
			)
		else:
...
func write_entity_spawn_command(p_entity_id: int, p_network_writer: Object) -> Object:
	...
	var entity_instance: Node = network_entity_manager.get_network_instance_for_instance_id(
		p_entity_id
	)
	...
	for entity in entities:
		if entity.is_inside_tree():
			var entity_command_network_writer: Object = create_entity_command(
				network_constants_const.SPAWN_ENTITY_COMMAND,
				entity.network_identity_node.network_instance_id
			)
	...
	if (
		...
		if network_entity_ids_pending_spawn.size():
			...
			for entity_id in network_entity_ids_pending_spawn:
				var entity_instance: Node = (
					network_manager
					. network_entity_manager
					. get_network_instance_for_instance_id(entity_id)
				)
		...
		for synced_peer in synced_peers:
			...
			if p_id == network_manager.session_master:
				...
				for entity_id in network_entity_ids_pending_spawn:
					...
					if ignore_list.has(
						network_manager.network_entity_manager.get_network_instance_for_instance_id(
							entity_id
						)
					):
						continue
...
func decode_entity_spawn_command(p_packet_sender_id: int, p_network_reader: Object) -> Object:
	...
	var multiplayer_authority: int = network_entity_manager.read_entity_multiplayer_authority(
		p_network_reader
	)
	if p_network_reader.is_eof():
		NetworkLogger.error("decode_entity_spawn_command: eof!")
	...
	if packed_scene == null:
		NetworkLogger.error(
			(
				"decode_entity_spawn_command: received invalid packed_scene for path {scene_path}!"
				. format({"scene_path": scene_path})
			)
		)
		return null
...
func decode_entity_transfer_master_command(
	...
	if network_entity_manager.network_instance_ids.has(instance_id):
		var entity_instance: Node = (
			network_entity_manager.network_instance_ids[instance_id].get_entity_node()
		)
