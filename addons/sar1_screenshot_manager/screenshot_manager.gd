...
const screenshot_manager_const = preload(
	"res://addons/sar1_screenshot_manager/screenshot_manager.gd"
)
...
static func _incremental_screenshot(p_info: Dictionary) -> Dictionary:
	...
	while FileAccess.file_exists(
		(
			screenshot_path_and_prefix
			+ str(screenshot_number).pad_zeros(INCREMENTAL_DIGIT_LENGTH)
			+ ".png"
		)
	):
		screenshot_number += 1
