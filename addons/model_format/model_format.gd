...
static func build_model_trees(p_root_node: Node3D) -> Dictionary:
	...
	for visual_node in model_node_lists.visual:
		var transform: Transform3D = node_util_const.get_relative_global_transform(p_root_node, visual_node)
	...
	for physics_node in model_node_lists.physics:
		var transform: Transform3D = node_util_const.get_relative_global_transform(p_root_node, physics_node)
