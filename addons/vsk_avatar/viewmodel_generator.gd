...
static func add_vertex(
	p_surface_tool: SurfaceTool, p_mesh_data_tool: MeshDataTool, p_vertex_id: int
) -> SurfaceTool:
...
static func get_recursive_children_for_bone_id(
	p_skeleton: Skeleton3D, p_bone_id: int
) -> PackedInt32Array:
	var valid_ids: Array = []
	...
	for mesh_surface_idx in range(0, mesh_surface_count):
		...
		for face_idx in range(0, mesh_data_tool.get_face_count()):
			...
			for face_vertex_idx in range(0, 3):
				vertex_ids[face_vertex_idx] = (mesh_data_tool.get_face_vertex(
					face_idx, face_vertex_idx
				))

				if vertex_table.has(vertex_ids[face_vertex_idx]):
					continue
				else:
					var bone_ids: PackedInt32Array = mesh_data_tool.get_vertex_bones(
						vertex_ids[face_vertex_idx]
					)
					var bone_weights: PackedFloat32Array = mesh_data_tool.get_vertex_weights(
						vertex_ids[face_vertex_idx]
					)
