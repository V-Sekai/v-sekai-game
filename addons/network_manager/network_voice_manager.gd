...
func decode_voice_command(p_packet_sender_id: int, p_network_reader: Object) -> Object:
	...
	if not network_manager.server_dedicated:
		network_manager.voice_packet_compressed.emit(
			sender_id, encoded_sequence_id, encoded_voice_byte_array
		)
		...
		for voice_buffer in voice_buffers:
			...
			for synced_peer in synced_peers:
				...
				network_writer_state = encode_voice_buffer(
					...
					(
						!network_manager.is_relay()
						and (synced_peer != network_constants_const.SERVER_MASTER_PEER_ID)
					)
				)
