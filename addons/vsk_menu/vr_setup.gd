...
func _ready() -> void:
	...
	setup_menu_button(turning_mode_button,\
	VRManager.vr_user_preferences.turning_mode,\
	VRManager.turning_mode_names)
	if (
		turning_mode_button.get_popup().id_pressed.connect(self._on_turning_mode_changed)
		!= OK):
		printerr("Could not connect 'id_pressed'!")
	...
	setup_menu_button(movement_type_button,\
	VRManager.vr_user_preferences.movement_type,\
	VRManager.movement_type_names)
	if (
		movement_type_button.get_popup().id_pressed.connect(self._on_movement_type_changed)
		!= OK):
		printerr("Could not connect 'id_pressed'!")
