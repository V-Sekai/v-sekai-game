...
static func create_pose_track_for_humanoid_bone(
	...
	p_animation.rotation_track_insert_key(
		track_index, 0.0, p_transform.basis.get_rotation_quaternion()
	)
...
static func create_animation_from_hand_pose(
	p_root_node: Node, p_skeleton: Skeleton3D, p_hand_pose: RefCounted
) -> Animation:
...
static func setup_animation_from_hand_pose_dictionary(
	...
	p_root_node: Node,
	p_skeleton: Skeleton3D,
	p_pose_dictionary: Dictionary
) -> void:
	for key in p_pose_dictionary:
		setup_animation_from_hand_pose(
			p_animation_player, p_root_node, p_skeleton, key, p_pose_dictionary[key]
		)
...
static func setup_default_hand_animations(
	...
	# Hand Animation
	var hand_pose_default_const = load(
		"res://addons/vsk_avatar/hand_poses/hand_pose_default_pose.tres"
	)
	...
	var hand_pose_thumbs_up_const = load(
		"res://addons/vsk_avatar/hand_poses/hand_pose_thumbs_up.tres"
	)
...
static func setup_animation_tree_hand_blend_tree(
	...
	var default_avatar_tree_const = load(
		"res://addons/vsk_avatar/animation/default_avatar_tree.tres"
	)
