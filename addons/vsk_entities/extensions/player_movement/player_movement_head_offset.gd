...
func execute(p_movement_controller: Node, p_delta: float) -> bool:
	...
	if (
		!get_character_body(p_movement_controller)
		or !get_xr_origin(p_movement_controller)
		or !get_xr_camera(p_movement_controller)
	):
		return false
	...
	var camera_offset: Vector2 = (camera_position - _previous_camera_position).rotated(
	...
	_velocity_accumulation += (
		Vector3(camera_offset.x, 0.0, camera_offset.y) * Engine.physics_ticks_per_second
	)
	...
	_velocity_accumulation -= (
		Vector3(distance_travelled.x, 0.0, distance_travelled.z) * Engine.physics_ticks_per_second
	)
