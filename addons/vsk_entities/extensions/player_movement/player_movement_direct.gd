...
func _get_action_rotation(p_movement_controller: Node) -> Vector2:
	var overall_rotation: float = (
		get_xr_origin(p_movement_controller).transform.basis.get_euler().y
		+ get_xr_camera(p_movement_controller).transform.basis.get_euler().y
	)
...
func execute(p_movement_controller: Node, p_delta: float) -> bool:
	...
	get_character_body(p_movement_controller).velocity = (
		Vector3(0.0, get_character_body(p_movement_controller).velocity.y, 0.0)
		+ (Vector3(rotated_input.x, 0.0, rotated_input.y) * speed) * Engine.physics_ticks_per_second
	)
