...
# vsk_editor_upload_panel_contents.gd
...
var user_content_type: int = VSKTypes.UserContentType.Unknown
...
func update_user_content_data(p_dictionary: Dictionary, p_updating_content: bool) -> void:
	...
	if updating_content:
		...
		get_node(description_text_edit_path).text = user_content_data.get(
				...
				if camera:
					...
					if new_preview_texture:
						...
						RenderingServer.viewport_attach_camera(
							viewport.get_viewport_rid(), camera.get_camera_rid()
						)
						...
						RenderingServer.request_frame_drawn_callback(
							func():
								RenderingServer.viewport_set_update_mode(
									...
									RenderingServer.VIEWPORT_UPDATE_DISABLED
								)
						)
		else:
...
static func _update_viewport_from_project_settings(p_viewport: SubViewport) -> SubViewport:
	...
	new_viewport.msaa_3d = ProjectSettings.get_setting("rendering/anti_aliasing/quality/msaa_3d")
	new_viewport.screen_space_aa = ProjectSettings.get_setting(
	...
	new_viewport.use_debanding = ProjectSettings.get_setting(
