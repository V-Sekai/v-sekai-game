...
static func get_transform_for_humanoid_bone(
	p_skeleton: Skeleton3D, p_humanoid_bone_name: String
) -> Transform3D:
...
static func generate_hand_pose_from_skeleton(
	p_skeleton: Skeleton3D, p_right_hand: bool
) -> Animation:
	...
	hand_pose_dict["LittleDistal"] = get_transform_for_humanoid_bone(
		p_skeleton, hand_prefix + "LittleDistal"
	)
	...
	for finger_bone in hand_pose_dict:
		for this_hand_prefix in ["Left", "Right"]:
			...
			hand_pose_anim.track_set_path(
				track, NodePath("%GeneralSkeleton:" + this_hand_prefix + finger_bone)
			)
