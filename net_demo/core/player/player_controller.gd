...
const camera_holder_const = preload("res://net_demo/core/player/camera_holder.gd")
...
func _process_rotation(p_movement_vector: Vector2) -> void:
	...
	if direction_distance > 0.0:
		var camera_pivot_node: Node3D = camera_holder_node.get_node_or_null(camera_holder_node.camera_pivot)
...
func _process_movement(p_delta: float, p_movement_vector: Vector2, p_is_sprinting: bool) -> void:
	...
	velocity = (applied_gravity_vector + (horizontal_velocity * (Vector3.ONE - up_direction)))
