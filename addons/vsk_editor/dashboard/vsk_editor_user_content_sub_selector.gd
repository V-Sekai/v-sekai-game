...
# vsk_editor_user_content_sub_selector.gd
# SPDX-License-Identifier: MIT
@tool
extends Control
class_name VSKEditorUserContentSubSelector

const vsk_user_content_selector_const = preload("vsk_editor_user_content_selector.gd")
...
func _reload_public_avatars() -> Dictionary:
	...
	if GodotUroHelper.requester_result_is_ok(async_result):
		...
		return_dict["message"] = GodotUroHelper.get_full_requester_error_string(
			async_result
		)
	"""
