...
func _process(delta: float):
	...
	if current_fadeout_rotate_delay <= 0:
		current_fadeout_rotate_amount = max(
			0.0, current_fadeout_rotate_amount - delta / fadeout_time
		)
	...
	if len(running_timestamps) > 1:
		var camera_origin_diff: Vector3 = (
			running_camera_xforms[0].origin - running_camera_xforms[-1].origin
		)
		var camera_basis_diff: Basis = (
			running_camera_xforms[-1].basis.inverse() * running_camera_xforms[0].basis
		)
		camera_origin_diff.y = 0
		var origin_origin_diff: Vector3 = (
			xr_origin_node.transform.basis
			* (running_origin_xforms[0].origin - running_origin_xforms[-1].origin)
		)
		var origin_basis_diff: Basis = (
			running_origin_xforms[-1].basis.inverse() * running_origin_xforms[0].basis
		)
		...
		if not camera_basis_diff.is_equal_approx(Quaternion.IDENTITY):
			camera_ang_speed = Quaternion.IDENTITY.angle_to(
				camera_basis_diff.get_rotation_quaternion()
			)
		var origin_ang_speed: float = 0
		if not origin_basis_diff.is_equal_approx(Quaternion.IDENTITY):
			origin_ang_speed = Quaternion.IDENTITY.angle_to(
				origin_basis_diff.get_rotation_quaternion()
			)
		...
		var vignette_alpha: float = clamp(
			max(move_vignette_amount, rotate_vignette_amount) * 5.0, 0.0, 1.0
		)
		var new_tunnel_visible: bool = (
			vignette_alpha > 0.001
			and enable_tunnel
			and (xr_camera_node.get_viewport().use_xr or preview_vignette > 0)
		)
		...
		var calculated_fov = max(
			iris_fov_limit, xr_camera_node.fov - max(calculated_move_fov, calculated_rotate_fov)
		)
